    ...
    describe ClusterRestart do
      ...
      context 'run_next' do
        ...
        it 'execute next with 4 brokers and 3 partitions' do
          ...
          command.run_next
          expect(command.session.broker_states['4']).to eq(Session::STATE_PENDING)
        end
        ...
        context 'run_good' do
          ...
          it 'set one broker to be restarted' do
            ...
            expect(session).to receive(:save!)
            command.restart('1')
            expect(command.session.broker_states['1']).to eq(Session::STATE_RESTARTED)
          end
        end
      end
    end
